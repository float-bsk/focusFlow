...
const Navbar = () => {
    ...
    return <nav className="relative w-full p-2 h-16 top-0 left-0 right-0 flex items-center-safe bg-amber-100 font-poppins text-blue-500 ">
        ...
        {/* Mobile Menu */}
        {
            open &&
            <div 
            className={`md:hidden flex flex-col gap-4 p-4 z-10 text-start right-0 top-16 bg-amber-100 rounded-lg shadow absolute
                menu-animate
            `}>
                <Link to="/">Home</Link>
                <Link to="/stats">Stats</Link>
...
const Logo = () => {
    ...
    return <header className="grow">
        <section className="font-poppins p-2  w-fit rounded-md grow-3">
